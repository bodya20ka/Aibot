...
export default function App() {
  ...
  const sendMessage = async () => {
    ...
    if (user) {
        ...
        if (querySnapshot) {
            const userKeywords = input.toLowerCase().split(' ').filter(w => w.length > 3);
            querySnapshot.forEach((doc) => {
                ...
                const isRelevant = userKeywords.length === 0 || 
                                   userKeywords.some(kw => filename.includes(kw) || content.toLowerCase().includes(kw));
                
                if (isRelevant) {
                    const truncatedContent = content.length > 800 ? content.substring(0, 800) + "... (truncated)" : content;
                    knowledgeContext += `\nDocument: ${data.filename}\nContent:\n${truncatedContent}\n`;
                }
            });
            // Final cap for entire knowledge context
            if (knowledgeContext.length > 7000) {
                knowledgeContext = knowledgeContext.substring(0, 7000) + "... (total context truncated)";
            }
        }
    }
